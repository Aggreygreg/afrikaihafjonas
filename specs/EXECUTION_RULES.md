...
7a. **True test count = 176** (was thought to be 161/123/167, then 170 before ProviderTranslation tests). `config/tests.py` has 6 tests that are easy to omit — **always include it in the pytest path**. Per-app: services 28, site_config 118, bookings 18, providers 6, config 6; `users`/`payments` test modules are empty stubs. Full command: `set "DJANGO_SETTINGS_MODULE=config.settings" && set "PYTHONIOENCODING=utf-8" && .venv\Scripts\python.exe -m pytest apps/services/tests.py apps/site_config/tests.py apps/bookings/tests.py apps/providers/tests.py apps/users/tests.py apps/payments/tests.py config/tests.py -q`
